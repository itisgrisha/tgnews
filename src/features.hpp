...
public:
    FeaturesGenerator() {
        auto models_prefix = fs::path("models");
        auto meta_prefix = fs::path("meta");
        for (const auto& lang : kLangs) {
            auto model_name = std::string(lang).append(".udpipe");
            auto model_path = (models_prefix / lang / model_name).string();
            ...
            lang2lemmas_dict_[lang] = ReadLemmas((meta_prefix / lang / "Lemmas.txt"));
        }
    ...
    DocFeatures GenerateFeatures(const HTMLDocument& doc, int sentences_count) {
        ...
        auto& lemmas_dict = lang2lemmas_dict_.at(lang);
        ...
        UpdateUDPipe(&features, model, reader, doc.GetText(),
            {"text:upostags", "text:feats", "text"}, lemmas_dict, sentences_count);
        UpdateUDPipe(&features, model, reader, doc.GetMeta("og:title"),
            {"title:upostags", "title:feats", "title"}, lemmas_dict, sentences_count);
...
private:
    ...
    void UpdateUDPipe(DocFeatures* features,
                      ...
                      const std::unordered_map<std::string, std::shared_ptr<BOWDict>>& lemmas_dict,
        ...
        auto features_lemmas_dict = lemmas_dict.at(features_names[2]);
        for (int total_sentences = 0;
            ...
            for (auto word = sentence_.words.begin()+1; word != sentence_.words.end(); ++word) {
                ...
                if (lemma_upostags.find(word->upostag) != lemma_upostags.end()) {
                    UpdateLemma(features, word->lemma, features_lemmas_dict, features_names[2]);
                }
    ...
    std::unordered_map<std::string, std::unordered_map<std::string, std::shared_ptr<BOWDict>>> lang2lemmas_dict_;
